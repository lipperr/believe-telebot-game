...
with Image.open("Users/annal/believebot/split_new/cards/cell.png") as im_cell:
    x, y = im_cell.size
dx = x // 10
dy = y // 3
...
async def take_photo_of_the_field(game: Game, player: Player, cur, prev, believe, new_step):
    with Image.new('RGB', field_sides(),
        ...
        for i in range(player.number + 1, len(game.players_in_game) + 1):
            with Image.open("Users/annal/believebot/split_new/cards/user_{}.png".format(i)) as im:
                new_im.paste(im, get_ind(len(game.players_in_game), index, 0))
            with Image.open("Users/annal/believebot/split_new/cards/cell_{}.png".format(
                    len(game.players_in_game[i - 1].cards))) as im:
        ...
        if len(player.cards) <= 32:
            for i in range(len(player.cards)):
                ...
                with Image.open("Users/annal/believebot/split_new/cards/{}.png".format(
                        SUITS_STR[current_card])) as im:
                    new_im.paste(im, card_position_at_field(i, len(player.cards)))
        else:
            with Image.open("Users/annal/believebot/split_new/cards/cell_{}.png".format(len(player.cards))) as im:
                new_im.paste(im, (((x + dx) * 4 + 4 * y) // 2 - x // 2, 5 * dy + 3 * y))
        with Image.open("Users/annal/believebot/split_new/cards/your_cards.png") as im:
            new_im.paste(im, (x + dx + 2 * y, 6 * dy + 4 * y - y - 4 * dy))
        if new_step:
            if game.is_game_open:
                with Image.open(f"Users/annal/believebot/split_new/cards/step_user_{game.cur_player}.png") as im:
            ...
            if game.current_card_value != "" and len(game.cards_in_game) > 0:
                ...
                with Image.open(
                        "Users/annal/believebot/split_new/cards/cell_{}.png".format(len(game.cards_in_game))) as im:
                    new_im.paste(im, card_position_of_bid(1, 1))
        else:
            if believe:
                with Image.new('RGB', (350, 35), (22, 132, 66)) as img:
                    draw = ImageDraw.Draw(img)
                    font = ImageFont.truetype("Users/annal/believebot/split_new/cards/arial.ttf", 25)
