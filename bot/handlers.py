...
async def hello_message(message):
    await bot.send_message(message.chat.id, f'Привет! Ты зашел в игру верю не верю.\n'
                                            f'Сейчас я подберу для тебя свободную игру, чтобы ты мог '
                                            f'анонимно играть с другими участвника')
...
async def end_game(message: [types.Message]):
    ...
    if manager.user_game.get(user_id) is not None and manager.user_game.get(user_id).is_game_open:
        ...
        for pl in game.players_in_game:
            if pl.id == message.from_user.id:
                await bot.send_message(user_id, 'Вы вышли из игры! Игра закончилась!', reply_markup=markup_reply)
            else:
                await bot.send_message(pl.id, f'Игра закончилась, так как игрок №{player.number} вышел из нее!',
...
async def rules(message):
    ...
    with open('rules.txt') as f:
        await bot.send_message(chat_id, f.readlines())
...
async def join_game(message: [types.Message]):
    ...
    if game is not None:
        ...
        if message.from_user.id == game.players_in_game[game.cur_player - 1].id:
            ...
            await bot.send_message(message.from_user.id, text=f'Игрок {game.cur_player} сделай ход',
                                   reply_markup=markup)
            await game.check()
        ...
        if not flag:  # не поверил и оказался прав
            if len(game.players_in_game[game.cur_player - 1].cards) == 0:
                ...
                for pl in game.players_in_game:
                    ...
                    pl.num_of_games += 1
                    if pl.id != winner.id:
                        await bot.send_message(pl.id, text=f'Игрок {winner.number} победил! Игра окончена')
                    else:
                        pl.num_of_wins += 1
                await bot.send_message(winner.id, text=f'Поздравляем! Вы победили! Игра окончена')
                clear_data(game)
...
async def options(game, player_id):
    ...
    await bot.send_message(player_id, f"Игрок {game.cur_player} сделай ход:", reply_markup=markup)
